...
function extraercupon(datosForm){
    var datos = datosForm.split("&");
        if (datos[3] == "cupon=55555"){
    ...
    <html>
        ...
        <body>
            ...
            <form method="POST">
                ...
                Código Cupón<input type="text" name="cupon"/></br>
...
//De esta manera identificamos la petición AJax ya que debe contener /search en la url
       ...
       if (req.method === 'POST'){
            ...
            data = chunk.toString();
            var precio = valoresCookie.length*10
            var cuponValido = extraercupon(data);

            reqForm = createReqForm();
            if (cuponValido == true){
                var descuento = precio *0.15;
                var preciodescuento = precio - descuento
                reqForm += `<p>El cupón es valido se te aplicará un 15% de descuento en tu compra</p>`
                reqForm += `<p>Total=` + precio + `</p>`
                reqForm += `<p>Total con descuento =` + preciodescuento + `</p>`

            }else{
                reqForm += `<p>Tu cupón está caducado. NO es válido</p>`
                reqForm += `<p>Total=` + precio + `</p>`

            }
            reqForm += `<p>`+ data +`</p>`

            reqForm += createEndForm(cuponValido);
